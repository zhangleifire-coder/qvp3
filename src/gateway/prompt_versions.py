...
# 自然真实感底座（2026-08-24 依 836 张人工满意样例提炼：
# 低饱和莫兰迪暖调 / 曲线分割版式 / 写实摄影质感 / 极简留白——
# 目标是出图贴近人工用 ChatGPT Image 做的卡片：舒适、克制、像杂志页不像模板）
_SHARED_IMAGE_STYLE = (
    "竖版3:4图文卡片。整体观感：自然、真实、舒适、克制，像杂志内页而非模板拼贴。"
    # 版式：曲线分割 + 呼吸感
    "版式：文字区与图片区之间用柔和的弧线或曲线自然过渡，"
    "忌生硬矩形拼贴；留白充足（浅色背景至少占画面四成），视觉有呼吸感。"
    # 色调：低饱和暖调
    "色调：低饱和莫兰迪暖调（奶油米、米金、香槟金、奶白、浅卡其、雾霾蓝绿等），"
    "整体柔和统一，严禁高饱和艳色与强对比撞色。"
    # 质感：写实摄影感（自然真实的核心）
    "质感：画面主体用写实摄影或高精度产品渲染呈现——柔和自然光或影棚柔光、"
    "浅景深、细腻材质与柔和阴影，地面可有轻微倒影；"
    "忌扁平卡通、忌粗糙手绘笔触、忌塑料感过强的伪3D。"
    # 文字：层级清晰、深字压浅底
    "文字：大号加粗黑体标题（约为正文字号的2.5倍），层级分明；"
    "深色文字直接压在浅色背景上，每行是短句，要点间用细线或小色条分隔。"
    ...
    # 装饰：极简
    "装饰：极简克制，只用细线、小色块或细金边点缀，不堆贴纸、不用花哨边框。"
...
_PAGE_LAYOUTS = [
    ...
    "本页是清单页：卡片式分栏布局，信息分成2-4块排列，每块一个小标题，"
