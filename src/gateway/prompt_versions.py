...
# 自然真实感底座（2026-08-24 首轮 4 张 + 2026-08-31 扩样 26 张，共 30 张人工样例提炼：
# 奶油米低饱和暖调 30/30 / 写实摄影或产品渲染 29/30（扁平插画 0 张）/ 深棕字压米底 30/30 /
# 留白 4-6 成 / 直线+暖棕细分隔线为主流（弧线约 1/3）/ 美食生活类偏好衬线宋体大标题 /
# 暖棕细线+圆角卡片边框+要点小圆图标+关键信息暖橘强调——目标：舒适、克制、像人工做的卡片）
_SHARED_IMAGE_STYLE = (
    "竖版3:4图文卡片。整体观感：自然、真实、舒适、克制，像杂志内页而非模板拼贴。"
    # 版式：图文二分 + 细线/弧线过渡 + 呼吸感
    "版式：文字区与图片区二分结构（上文下图或上图下文），"
    "以水平暖棕细分隔线或柔和弧线过渡，忌生硬矩形拼贴；"
    "留白充足（浅色背景至少占画面四成），视觉有呼吸感。"
    # 色调：低饱和暖调（图片内部允许天然色）
    "色调：背景用低饱和暖调（奶油米、浅米黄、米金、香槟金、奶白、浅卡其），"
    "整体柔和统一，严禁背景高饱和艳色与强对比撞色；"
    "图片主体内部允许保留食材、果蔬等天然的适度色彩。"
    # 质感：写实摄影感（自然真实的核心）
    "质感：画面主体用写实摄影或高精度产品渲染呈现——柔和自然光或影棚柔光、"
    "浅景深与轻微虚化、细腻材质与柔和阴影，地面可有轻微倒影；"
    "忌扁平卡通、忌粗糙手绘笔触、忌塑料感过强的伪3D。"
    # 文字：层级清晰、深棕字压米底（同色系柔和）、暖橘强调
    "文字：大号标题（约为正文字号的2.5倍），层级分明；"
    "标题按题材气质选加粗黑体（科技/汽车/数码）或衬线宋体（美食/生活/自然科普）；"
    "深棕或深咖色文字压浅色背景（与米底同色系呼应、柔和而非纯黑），"
    "关键数字或关键词可用暖橘色强调；每行是短句，要点间用暖棕细线或小色条分隔。"
    "所有文字必须清晰可读、标准中文字体，禁止艺术化变形、阴影、描边、透视扭曲，"
    "正文统一基线对齐、可印刷级清晰；图中文字总计不超过40字（字少才不易出错），"
    "每页只突出一个核心信息点，其余信息留给正文，不出现字号过小的文字，"
    ...
    # 装饰：极简同色系（暖棕细线/圆角卡边框/小圆图标）
    "装饰：极简克制且同色系——暖棕细分隔线、圆角卡片式边框、"
    "每条要点前可配一枚统一的小圆图标，不堆贴纸、不用花哨边框。"
...
_PAGE_LAYOUTS = [
    "本页是封面页：主视觉大图占画面约三分之二（写实摄影或高精度渲染质感），"
    "大标题置顶部，副标题只一行，整体留白充足。",
    "本页是要点页：上文下图布局，正文拆成2-3个短句要点纵向排列，"
    "每条要点前配一枚统一的小圆图标，要点间用暖棕细线或小色块分隔，"
    "文字区与图片区以水平细线或柔和弧线过渡。",
    "本页是特写页：主体特写充满画面（浅景深、柔和光效），"
    "文字只放在底部约四分之一的浅色横条区域内。",
    "本页是清单页：圆角卡片式分栏布局，信息分成2-4块排列，每块一个小标题，"
    "块间留明显间距，卡片底色与背景同为低饱和浅色。",
    "本页是场景页：全幅写实场景铺满画面（自然光、真实质感），"
    "文字置于顶部浅色留白区内，图与文字以弧线或斜线自然衔接。",
    "本页是总结页：居中大字结论，下方最多两行小字，视觉收尾干净利落。",
]

IMAGE_PROMPTS = {
    "general": "通用科普/教程配图，纯 AI 生成、无参考图，主体以写实摄影质感呈现。" + _SHARED_IMAGE_STYLE + "本页文案：{page_body}",
...
# 2026-08-26 收紧字数：图上文字太多是「文字扭曲」与「信息过载」的共同源头——
# 每页只保留一个核心信息点，短句、字少，其余细节留给正文
PAGES_PROMPT = """你是小红书图文编辑。把下面的文章改写成 6 页图上文案，用于竖版图文卡片。
要求：
1. 输出严格的 JSON 数组，恰好 6 个字符串，不要输出任何其他文字、解释或 markdown 代码围栏。
2. 第 1 页是封面：主标题（12-20 字）+ 一句钩子，要短、有冲击力。
3. 第 2-5 页每页只讲一个核心信息点：小标题 + 1 句干货（整页 25-50 字），
   忠于原文的事实与数据，不得编造；把次要信息留给正文，宁短勿长。
4. 第 6 页是结尾：一句总结 + 适合谁/行动建议（20-40 字）。
5. 全部纯文本：不用 markdown 符号（#、*、- 等），不用 emoji，无绝对化表述，中文标点。
6. 每页文字都要语句完整通顺、能直接排版在图片上；整页文字量越少越好（图上字少才清晰不易出错）。
...
3. 25-50 字（只保留一个核心信息点，宁短勿长），语句完整通顺，能直接排版在图片上。
