...
async def fetch_image_bytes(image_url: str) -> tuple:
    ...
    if image_url.startswith("/static/"):
        ...
        # 同步磁盘读丢线程池，避免阻塞事件循环（SSE/其他请求延迟抖动）
        return await asyncio.to_thread(local.read_bytes), ctype
    ...
    # 共享 client 承载连接池（P1-7）；follow_redirects 按请求覆盖保持原语义
    resp = await get_client("ocr", timeout=120).get(
...
async def ocr_image(image_url: str) -> dict:
    ...
    from src.gateway.http_client import get_client
    resp = await get_client("ocr", timeout=120).post(
