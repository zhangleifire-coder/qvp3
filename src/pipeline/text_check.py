...
必查项——概念偷换：逐个圈出 Query 的核心概念词（对比轴两侧、风格/流派名、品类名、题面点名的对象），检查每个概念词是否在正文中原样出现并被正面回答。若正文用自选的同义单品/例子顶替了题面概念（如问「冷淡风和法式风耳环」却全程只写「小圆环和珍珠耳坠」、只字不提「法式风」），判 warn 并在 reason 写明「概念偷换：题面X被替换为Y」。
必查项——概念收敛：题面概念是风格/流派等集合概念时，检查正文是否覆盖了该概念的多种典型形态（至少 2-3 种）。若概念词虽然出现、但论述与选购建议全程只围绕其中单一单品/单一形态展开（如「法式风」虽然提到、但通篇只是珍珠耳环的选型参数），判 warn 并在 reason 写明「概念收敛：风格X被收敛为单品Y」。
