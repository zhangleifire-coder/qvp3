...
class Settings(BaseSettings):
    ...
    # 混合生图（程序文字版式 + AI 无文字画面）：2026-09-23 用户裁决弃用
    # 「呆板图文方式」，默认走模型整卡直出（案例风格库关键字驱动，见
    # docs/案例风格库与生图关键字重设计-20260923.md）；true 可回退混合链
    image_compose_mode: bool = False
    # 单候选生图（2026-09-21 用户决策）：文字铁律+VL 质检已保障质量，
    # 双候选 OCR 选优的收益不抵成本；true=恒单候选（可回滚）
    image_single_candidate: bool = True
    # 交付页数：0922 参考样式标准 = 5 页（用户裁决 2026-09-22，每页一节段落）；
    # 5|6 可配，其余值回落 5。注意：deprecated 的 monolith 路径仍固定 6 页。
    page_count: int = 5
