...
### 风格自适应（普通/CSV 导入自动定风格，迁移 008，2026-08-24）

普通逐行导入与 CSV 导入的任务不再使用固定生图风格——Agent 在检索证据后自适应判定：

- **内容风格**：结合 query 与检索到的信息，从 解读·经验分享/测评实测/攻略教程/
  避坑指南/观点杂谈 中判定（多个适配随机选）；组合导入任务已显式指定风格的
  不覆盖（fixed_style 注入，content_style 直接回填指定值）
- **图片整体视觉风格**：从 8 种风格库选择（治愈暖彩/真实摄影/扁平极简/3D渲染/
  手绘线稿/杂志编辑/信息图表/复古印刷），多适配随机选——同样内容多次生成可
  选到不同风格；生成配图时把生图模板中的固定风格句「坚韧治愈风、高清、极简高级」
  替换为所选风格描述词，6 图整体风格统一
- **落库展示**（迁移 008 `tasks.gen_image_style`）：图片风格始终记录本轮判定，
  内容风格仅在任务未指定时回填；任务详情抽屉新增「配图：xx」标签
